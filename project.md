...
- **JSS 前硬擋**：每個病灶都要有長寬高＋至少一張照片才開得了 JSS。量不到／病人拒拍時，
...
- **判定集中在 `lib/clinicFlow.ts`**：動線頁、今日門診卡片、完成畫面三處共用同一套
  「什麼叫量完了」，各寫一次遲早不一致。
- **JSS 送出後回動線頁**顯示「本次收案完成」（問卷頁與 action 新增 `next` 參數，只收站內相對路徑）。
...
### 2026-08-20：JSS 追蹤評估表維持全部由醫師填

曾評估把第 5 疼痛／第 6 瘙癢移給病人自填（純主觀症狀），**使用者確認不移**——
那兩題是醫師問診的內容，計分與匯出欄位維持原樣，不需改動。
...
- 斷點統一成 `min-[560px]`（不再是 `md`）：以上用可捲的中段，以下捲動視窗會窄到不能用，
  交給側邊選單。單一斷點，不會再有死區。
