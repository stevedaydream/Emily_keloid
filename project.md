...
3. **醫師評分**（JSS 疤痕診斷分類表 ＋ VSS，兩份都完成才算走完這一步）
...
- **VSS 與 JSS 擺同一關**（2026-08-20 補）：兩份都要醫師看同一顆病灶、摸同一個質地
...
2. **重新量測與拍照**（複用 `LesionMeasureFlow`）
3. **醫師評分**（VSS 每次；術後 12／24 個月加 SF-36 ＋ PSQI）
...
⚠️ 它**不是**尺寸的歷史紀錄，舊值仍會被覆蓋——完整時間序列是 pending.md E6，使用者決定先不做。
